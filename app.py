...
def top_navigation():
    """Thanh menu chính ở giữa, dạng nút vuông bo nhẹ, tối ưu mobile."""
    user = get_user(st.session_state.user["id"]) or st.session_state.user
    options = []
    ...
    if user_has_permission(user, "manage_users") or user_has_permission(user, "manage_roles"):
        options.append(("🛠️", "QUẢN TRỊ", "🛠️ ADMIN"))
    if not options:
        options = [("🏠", "THÔNG TIN", "🏠 Tổng quan")]

    current = st.session_state.get("top_menu", options[0][2])
    if current not in [x[2] for x in options]:
        current = options[0][2]

    st.markdown('<div class="top-nav-wrap"><div class="top-nav-title">MENU ĐIỀU HƯỚNG</div><div class="top-menu-grid">', unsafe_allow_html=True)
    cols = st.columns(len(options))
    for i, (icon, label, value) in enumerate(options):
        with cols[i]:
            active = value == current
            if st.button(f"{icon}  {label}", key=f"topmenu_{i}", use_container_width=True, type="primary" if active else "secondary"):
                st.session_state.top_menu = value
                st.rerun()
...
def main():
    ...
    st.markdown("""
    <div class="footer">
        🚕 TAXI CẦU VỒNG • Hệ thống đang hoạt động • Hoạt động 24/24* • GMT+7
    </div>
    """,unsafe_allow_html=True)
