...
def top_navigation():
    """Thanh menu chính nằm giữa phía trên trang."""
    user = get_user(st.session_state.user["id"]) or st.session_state.user
    options = []
    ...
    if not options:
        options = ["🏠 Tổng quan"]
    current = st.session_state.get("top_menu", options[0])
    if current not in options:
        current = options[0]
    st.markdown('<div class="top-nav-wrap"><div class="top-nav-title">🚕 MENU ĐIỀU HƯỚNG • TAXI CẦU VỒNG</div><div class="top-menu">', unsafe_allow_html=True)
    menu = st.radio("MENU", options, index=options.index(current), key="top_menu_radio", horizontal=True, label_visibility="collapsed")
    st.markdown('</div></div>', unsafe_allow_html=True)
    st.session_state.top_menu = menu
    return menu
