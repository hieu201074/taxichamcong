...
def init_db():
    ...
    try:
        ...
        # Không lưu tài khoản/mật khẩu Admin mặc định trong source code.
        # Cấu hình lần đầu qua biến môi trường:
        #   TAXI_ADMIN_USERNAME
        #   TAXI_ADMIN_PASSWORD
        # Sau khi tạo, mật khẩu chỉ được lưu dưới dạng hash trong database.
        admin_username = os.getenv("TAXI_ADMIN_USERNAME", "").strip()
        ...
        if admin_username and admin_password:
            ...
            if not admin:
                ...
                # Tài khoản được cấu hình qua môi trường luôn giữ quyền Admin.
                conn.execute(
                    "UPDATE users SET role='admin', approved=1, active=1 WHERE username=?",
                    (admin_username,)
                )
        else:
            # Không tự tạo Admin với username/password mặc định.
            # Nếu database đã có Admin, giữ nguyên tài khoản đó.
            admin = conn.execute(
                "SELECT id FROM users WHERE role='admin' ORDER BY id LIMIT 1"
            ).fetchone()
            if not admin:
                # Chỉ tạo Admin khi người quản trị cung cấp credentials qua ENV.
                pass
