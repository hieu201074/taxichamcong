...
# Luôn lưu database ở thư mục cố định cạnh file database.py, tránh bị tạo
# thành file taxi.db khác khi Streamlit đổi working directory.
DB_FILE = str(Path(__file__).resolve().parent / "taxi.db")
...
def init_db():
    ...
    try:
        ...
        if not admin:
            ...
            # Tài khoản admin gốc luôn giữ quyền admin.
            conn.execute(
                "UPDATE users SET role='admin', approved=1, active=1 WHERE username='admin'"
            )
