...
# Database riêng, tách khỏi mã nguồn để giảm nguy cơ lộ dữ liệu tài khoản.
# Thư mục này cũng nên được thêm vào .gitignore khi deploy GitHub.
BASE_DIR = Path(__file__).resolve().parent
# Có thể đặt TAXI_DATA_DIR trên hosting để trỏ tới ổ đĩa/database persistent.
# Nếu không đặt, dữ liệu nằm trong database_private/ và KHÔNG nên commit lên GitHub.
_configured_data_dir = os.getenv("TAXI_DATA_DIR", "").strip()
PRIVATE_DB_DIR = Path(_configured_data_dir).expanduser() if _configured_data_dir else (BASE_DIR / "database_private")
PRIVATE_DB_DIR.mkdir(parents=True, exist_ok=True)
DB_PATH = PRIVATE_DB_DIR / "taxi.db"
LEGACY_DB_PATH = BASE_DIR / "taxi.db"

# Nếu có taxi.db cũ cạnh code, tự sao chép sang vị trí mới một lần
# để giữ tài khoản/dữ liệu cũ. Không ghi đè DB riêng đã tồn tại.
...
def init_db():
    ...
    try:
        ...
        # Không hard-code mật khẩu admin trong source.
        # Trên hosting hãy đặt TAXI_ADMIN_PASSWORD trong Secrets/Environment Variables.
        admin_username = os.getenv("TAXI_ADMIN_USERNAME", "admin").strip() or "admin"
        admin_password = os.getenv("TAXI_ADMIN_PASSWORD", "")
        admin_name = os.getenv("TAXI_ADMIN_NAME", "Administrator").strip() or "Administrator"

        # Tìm đúng tài khoản admin theo username cấu hình trước.
        admin = conn.execute(
            "SELECT id FROM users WHERE username = ?", (admin_username,)
        ).fetchone()

        if admin:
            # Giữ nguyên mật khẩu/dữ liệu hiện có; chỉ bảo đảm quyền admin.
            conn.execute(
                "UPDATE users SET role='admin', approved=1, active=1 WHERE id=?",
                (admin["id"],),
            )
        else:
            # Nếu DB đã có một tài khoản admin khác thì giữ tài khoản đó,
            # tránh INSERT trùng username khi app khởi động lại.
            ...
            if existing_admin:
                conn.execute(
                    "UPDATE users SET approved=1, active=1 WHERE id=?",
                    (existing_admin["id"],),
                )
            elif admin_password:
                conn.execute("""
...
def add_trip(user_id, image_path, note=""):
    if get_today_trip_count(user_id) >= 300:
        return False, "Bạn đã đạt tối đa 300 chuyến hôm nay."
...
def approve_trip(trip_id, admin_id=None):
    ...
    try:
        ...
        if get_today_trip_count(trip["user_id"]) >= 300:
            return False, "Tài xế đã đạt 300 chuyến hôm nay."
...
def get_leaderboard():
    ...
    try:
        rows = conn.execute("""
            SELECT users.id, users.full_name, users.duty,
