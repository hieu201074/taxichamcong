...
def init_db():
    ...
    try:
        ...
        # Streamlit Cloud: cấu hình trong Settings -> Secrets:
        #   TAXI_ADMIN_USERNAME
        #   TAXI_ADMIN_PASSWORD
        # Hosting khác: có thể dùng Environment Variables cùng tên.
        ...
        if admin_username and admin_password:
            ...
            if not admin:
                ...
                # Nếu tài khoản đã tồn tại thì KHÔNG INSERT lại, tránh
                # sqlite3.IntegrityError do username UNIQUE.
                conn.execute("""
                    UPDATE users
                    SET role='admin', approved=1, active=1
                    WHERE id=?
                """, (admin["id"],))
        # Không có Secret/ENV thì không tự tạo tài khoản Admin mặc định.
        # Admin đã tồn tại trong taxi.db vẫn được giữ nguyên.
